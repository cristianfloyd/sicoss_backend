...
class SicossDatabaseSaver:
    ...
    def guardar_en_bd(
        ...
        try:
            ...
            resultado_guardado = self._ejecutar_guardado_bd(
                legajos_mapeados, periodo_fiscal
            )
            ...
            logger.info(
                f"✅ Guardado BD completado: {resultado['legajos_guardados']} legajos"
            )
            return resultado
    ...
    def generar_sicoss_bd(
        ...
        logger.info(
            f"🚀 Simulando generación SICOSS a BD para período {periodo_fiscal}"
        )
        ...
        try:
            ...
            if resultado_procesamiento.get("success", False):
                legajos_procesados = resultado_procesamiento.get(
                    "legajos_procesados", pd.DataFrame()
                )
                resultado_bd = self.guardar_en_bd(
                    legajos_procesados, periodo_fiscal, incluir_inactivos
                )
                ...
                logger.info("✅ Generación SICOSS a BD simulada completada")
    ...
    def _mapear_legajos_a_modelo(
        self, legajos: pd.DataFrame, periodo_fiscal: PeriodoFiscal
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.info(
                f"✅ Mapeo completado: {len(legajos_bd)} legajos mapeados con {len(legajos_bd.columns)} campos"
            )
            return legajos_bd
    ...
    def _convertir_tipos_bd_real(self, legajos_bd: pd.DataFrame):
        ...
        campos_texto = {"cuil": 11, "apnom": 40, "cod_os": 6, "prov": 50}

        for campo, max_len in campos_texto.items():
            if campo in legajos_bd.columns:
                legajos_bd[campo] = (
                    legajos_bd[campo].astype(str).str[:max_len].fillna("")
                )
        ...
        for campo in campos_enteros:
            if campo in legajos_bd.columns:
                legajos_bd[campo] = (
                    pd.to_numeric(legajos_bd[campo], errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
        ...
        for campo in campos_numericos:
            if campo in legajos_bd.columns:
                legajos_bd[campo] = pd.to_numeric(
                    legajos_bd[campo], errors="coerce"
                ).fillna(0.0)
    ...
    def _validar_restricciones_tabla(self, legajos_bd: pd.DataFrame):
        ...
        for campo in campos_not_null:
            if campo in legajos_bd.columns:
                ...
                if valores_nulos > 0:
                    logger.warning(
                        f"⚠️ Campo {campo} tiene {valores_nulos} valores nulos"
                    )
    ...
    def _convertir_tipos_bd(self, legajos_bd: pd.DataFrame):
        ...
        campos_decimales = [
            col
            for col in legajos_bd.columns
            if "importe" in col.lower() or "aporte" in col.lower()
        ]
    ...
    def _validar_datos_para_bd(self, legajos_bd: pd.DataFrame) -> bool:
        ...
        """
        logger.info(
            f"🔍 Validando {len(legajos_bd)} legajos para tabla afip_mapuche_sicoss"
        )
        ...
        campos_faltantes = [
            campo
            for campo in campos_requeridos_tabla
            if campo not in legajos_bd.columns
        ]

        if campos_faltantes:
            raise ValueError(
                f"Campos faltantes para tabla afip_mapuche_sicoss: {campos_faltantes}"
            )
        ...
        campos_importes = [
            col
            for col in legajos_bd.columns
            if col.startswith("rem_") or col in ["sac", "asig_fam_pag", "aporte_vol"]
        ]
    ...
    def _ejecutar_guardado_bd(
        self, legajos_bd: pd.DataFrame, periodo_fiscal: PeriodoFiscal
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"💾 Ejecutando guardado en {self.schema}.{self.tabla_sicoss} para {len(legajos_bd)} legajos"
        )
        ...
        try:
            ...
            estadisticas = self._generar_estadisticas_guardado(
                legajos_bd, periodo_fiscal
            )
            ...
            logger.info(
                f"✅ Guardado BD exitoso: {filas_guardadas} filas en {self.schema}.{self.tabla_sicoss} ({duracion:.2f}s)"
            )
            return resultado
    ...
    def _generar_estadisticas_guardado(
        self, legajos_bd: pd.DataFrame, periodo_fiscal: PeriodoFiscal
    ) -> Dict[str, Any]:
        """
        ...
        try:
            estadisticas = {
                ...
                "tamaño_mb": round(
                    legajos_bd.memory_usage(deep=True).sum() / 1024 / 1024, 2
                ),
                "periodo_fiscal": periodo_fiscal.periodo_str,
            }
            ...
            if "rem_total" in legajos_bd.columns:
                ...
                estadisticas["promedio_rem_total"] = float(
                    legajos_bd["rem_total"].mean()
                )
    ...
    def verificar_estructura_datos(
        self, periodo_fiscal: PeriodoFiscal, muestra: int = 3
    ) -> Dict[str, Any]:
        """
    ...
    def _ensure_table_exists(self, periodo_fiscal: PeriodoFiscal):
        ...
        try:
            ...
            if not resultado.empty and int(resultado.iloc[0]["existe"]) > 0:
                ...
                logger.warning(
                    f"⚠️ Tabla {self.schema}.{self.tabla_sicoss} no encontrada"
                )
                raise RuntimeError(
                    f"Tabla {self.schema}.{self.tabla_sicoss} no existe en BD"
                )
    ...
    def _filtrar_legajos_activos(self, legajos_bd: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if "nro_legaj" in legajos_bd.columns:
                mask_valido = (legajos_bd["nro_legaj"] > 0) & legajos_bd[
                    "nro_legaj"
                ].notna()
            else:
                # Si no hay columna nro_legaj, mantener todos los registros
                mask_valido = pd.Series(
                    [True] * len(legajos_bd), index=legajos_bd.index
                )
