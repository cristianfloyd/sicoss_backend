...
class SicossVerifier:
    ...
    def __init__(self, tolerancia: Optional[ToleranciaComparacion] = None):
        """
        Inicializa una instancia del verificador SicossVerifier con las
        tolerancias y la configuración de qué campos comparar según su tipo.

        Args:
            tolerancia (Optional[ToleranciaComparacion]): Objeto que define las tolerancias
                utilizadas para la comparación de los distintos tipos de campos.
                Si no se especifica, se utiliza la configuración por defecto.

        Atributos:
            tolerancia (ToleranciaComparacion): Tolerancias de comparación para distintos tipos de campos.
            campos_monetarios (set[str]): Conjunto de nombres de campos asociados a importes
                monetarios o remunerativos, que serán comparados usando la tolerancia monetaria.
            campos_enteros (set[str]): Conjunto de nombres de campos que representan valores enteros,
                que se comparan de manera exacta o con tolerancia de enteros.
            campos_booleanos (set[str]): Conjunto de nombres de campos cuyo valor es booleano
                o representa una condición verdadera/falsa.
        """
        ...
        self.campos_booleanos = {"SeguroVidaObligatorio", "trabajadorconvencionado"}

    def verificar_resultados(
        self,
        df_python: pd.DataFrame,
        df_php: pd.DataFrame,
        campos_criticos: Optional[List[str]] = None,
    ) -> ReporteVerificacion:
        """
        ...
        logger.info(
            f"📊 Comparando {len(campos_criticos)} campos en {len(df_python_prep)} legajos"
        )
        ...
        for campo in campos_criticos:
            ...
            resultados_campo = self._comparar_campo(df_python_prep, df_php_prep, campo)
    ...
    def _preparar_datos(
        self, df_python: pd.DataFrame, df_php: pd.DataFrame
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Prepara y normaliza los DataFrames de resultados para comparación uno a uno.

        Este método realiza las siguientes tareas:
            1. Crea copias independientes de los DataFrames originales.
            2. Verifica la existencia de la columna clave "nro_legaj" en ambos DataFrames.
            3. Ordena ambos DataFrames por la columna "nro_legaj" y reinicia los índices.
            4. Identifica la intersección de legajos existentes en ambos DataFrames.
            5. Filtra ambos DataFrames para conservar solo los legajos comunes.
            6. Normaliza los tipos de dato de las columnas relevantes mediante el método _normalizar_tipos.
            7. Devuelve ambos DataFrames listos para la comparación campo a campo.

        Args:
            df_python (pd.DataFrame): DataFrame con los resultados del sistema Python.
        ...
        Raises:
            ValueError: Si alguno de los DataFrames no contiene la columna "nro_legaj".
        """
        ...
        # Verificar existencia de la columna clave "nro_legaj"
        ...
        # Ordenar ambos DataFrames por 'nro_legaj' y resetear el índice
        df_py = df_py.sort_values("nro_legaj").reset_index(drop=True)
        df_php = df_php.sort_values("nro_legaj").reset_index(drop=True)

        # Obtener set con legajos comunes presentes en ambos DataFrames
        ...
        # Filtrar ambos DataFrames para conservar únicamente los legajos comunes
        df_py = df_py[df_py["nro_legaj"].isin(list(legajos_comunes))]
        df_php = df_php[df_php["nro_legaj"].isin(list(legajos_comunes))]

        # Realizar normalización de los tipos de dato en ambos DataFrames
        df_py = self._normalizar_tipos(df_py)
        df_php = self._normalizar_tipos(df_php)

        # Retornar ambos DataFrames preparados y normalizados
        return df_py, df_php
    ...
    def _obtener_campos_criticos(
        ...
        campos_excluir = {
            "CalculosProcessorCompleto",
            "TimestampCalculosProcessor",
            "ConceptosProcessorCompleto",
            "TimestampConceptosProcessor",
            "TopesProcessorCompleto",
            "TimestampTopesProcessor",
        }
    ...
    def _comparar_campo(
        self, df_python: pd.DataFrame, df_php: pd.DataFrame, campo: str
        ...
        for idx, row_py in df_python.iterrows():
            ...
            resultado = self._comparar_valores(
                campo, nro_legaj, valor_python, valor_php
            )
    ...
    def _comparar_valores(
        self, campo: str, legajo: int, valor_python: Any, valor_php: Any
        ...
        if pd.isna(valor_python) or pd.isna(valor_php):
            return ResultadoComparacion(
                ...
                porcentaje_diferencia=float("inf"),
                es_coincidente=False,
                tipo_diferencia="error",
            )
    ...
    def _comparar_monetario(
        self, campo: str, legajo: int, valor_python: float, valor_php: float
    ) -> ResultadoComparacion:
        ...
        return ResultadoComparacion(
            ...
            es_coincidente=es_tolerancia,
            tipo_diferencia=tipo,
        )

    def _comparar_entero(
        self, campo: str, legajo: int, valor_python: int, valor_php: int
    ) -> ResultadoComparacion:
        """Compara valores enteros (debe ser exacto)"""
        diferencia = abs(int(valor_python) - int(valor_php))
        porcentaje_diferencia = (
            (diferencia / max(abs(int(valor_python)), abs(int(valor_php))) * 100)
            if max(abs(int(valor_python)), abs(int(valor_php))) > 0
            else 0
        )
        ...
        return ResultadoComparacion(
            ...
            porcentaje_diferencia=porcentaje_diferencia,
            es_coincidente=es_coincidente,
            tipo_diferencia=tipo,
        )

    def _comparar_booleano(
        self, campo: str, legajo: int, valor_python: bool, valor_php: bool
    ) -> ResultadoComparacion:
    ...
    def _comparar_generico(
        ...
        try:
            ...
            porcentaje_diferencia = (
                (diferencia / max(abs(val_py), abs(val_php)) * 100)
                if max(abs(val_py), abs(val_php)) > 0
                else 0
            )
        ...
        tipo = (
            "exacto"
            if diferencia == 0
            else ("tolerancia" if es_coincidente else "error")
        )
    ...
    def _generar_reporte(
        self, resultados: List[ResultadoComparacion], tiempo_verificacion: float
    ) -> ReporteVerificacion:
        """Genera reporte completo de verificación"""

        total_comparaciones = len(resultados)
        coincidencias_exactas = sum(
            1 for r in resultados if r.tipo_diferencia == "exacto"
        )
        ...
        porcentaje_coincidencia = (
            (coincidencias_tolerancia / total_comparaciones * 100)
        ...
        recomendaciones = self._generar_recomendaciones(
            resultados, porcentaje_coincidencia
        )
    ...
    def _generar_estadisticas(
        self, resultados: List[ResultadoComparacion]
    ) -> Dict[str, Any]:
        ...
        diferencias_numericas = [
            r.diferencia for r in resultados if not np.isinf(r.diferencia)
        ]
        porcentajes_diferencia = [
            r.porcentaje_diferencia
            for r in resultados
            if not np.isinf(r.porcentaje_diferencia)
        ]
        ...
        return {
            "diferencia_promedio": np.mean(diferencias_numericas)
            if diferencias_numericas
            else 0,
            "diferencia_maxima": np.max(diferencias_numericas)
            if diferencias_numericas
            else 0,
            "diferencia_mediana": np.median(diferencias_numericas)
            if diferencias_numericas
            else 0,
            "porcentaje_diferencia_promedio": np.mean(porcentajes_diferencia)
            if porcentajes_diferencia
            else 0,
            "campos_con_mas_errores": sorted(
                campos_con_errores.items(), key=lambda x: x[1], reverse=True
            )[:5],
            "total_comparaciones": len(resultados),
    ...
    def _generar_recomendaciones(
        self, resultados: List[ResultadoComparacion], porcentaje_coincidencia: float
    ) -> List[str]:
        ...
        if porcentaje_coincidencia >= 99.5:
            recomendaciones.append(
                "✅ Excelente: Sistema Python altamente consistente con PHP legacy"
            )
        elif porcentaje_coincidencia >= 95.0:
            recomendaciones.append(
                "🟡 Bueno: Consistencia aceptable, revisar diferencias menores"
            )
        else:
            recomendaciones.append(
                "🔴 Crítico: Revisar inconsistencias significativas antes de producción"
            )
        ...
        if errores_criticos:
            ...
            if len(campos_problematicos) <= 3:
                recomendaciones.append(
                    f"🔧 Revisar cálculos en campos: {', '.join(campos_problematicos)}"
                )
            else:
                recomendaciones.append(
                    f"🔧 Revisar cálculos en {len(campos_problematicos)} campos problemáticos"
                )

        return recomendaciones

    def generar_reporte_html(
        self, reporte: ReporteVerificacion, archivo_salida: Optional[str] = None
    ) -> str:
        """Genera reporte HTML detallado"""
        if archivo_salida is None:
            archivo_salida = (
                f"reporte_verificacion_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
            )
