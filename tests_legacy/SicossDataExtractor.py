...
class SicossDataExtractor:
    ...
    def extraer_datos_completos(
        ...
        """
        logger.info(
            f"Iniciando extracción de datos para período {per_anoct}/{per_mesct}"
        )
        ...
        where_legajo_conceptos = self.construir_where_conceptos(
            df_legajos["nro_legaj"].tolist()
        )
        df_conceptos = self.extraer_conceptos_liquidados(
            per_anoct, per_mesct, where_legajo_conceptos
        )

        # 3. Extraer otra actividad
        logger.info("Extrayendo datos de otra actividad...")
        df_otra_actividad = self.extraer_otra_actividad(
            df_legajos["nro_legaj"].tolist()
        )

        # 4. Extraer códigos de obra social
        logger.info("Extrayendo códigos de obra social...")
        df_obra_social = self.extraer_codigos_obra_social(
            df_legajos["nro_legaj"].tolist()
        )
    ...
    def extraer_legajos(
        self, per_anoct: int, per_mesct: int, where_legajo: str
    ) -> pd.DataFrame:
        """Extrae datos básicos de legajos"""
        query = self.sql_queries.get_legajos_query(
            per_anoct, per_mesct, "'REPA'", where_legajo
        )
        return self.db.execute_query(query)

    def extraer_conceptos_liquidados(
        self, per_anoct: int, per_mesct: int, where_legajo: str
    ) -> pd.DataFrame:
        """Extrae conceptos liquidados con tipos de grupos"""
        query = self.sql_queries.get_conceptos_liquidados_query(
    ...
    def mostrar_estadisticas_extraccion(self, dataframes: Dict[str, pd.DataFrame]):
        ...
        if not dataframes["conceptos"].empty:
            ...
            logger.info(
                f"Promedio conceptos por legajo: {conceptos_por_legajo.mean():.1f}"
            )
...
class SicossDataProcessor:
    ...
    def _sumarizar_conceptos_por_legajo(
        ...
        conceptos_agrupados = (
            df_conceptos.groupby("nro_legaj").agg({"impp_conce": "sum"}).reset_index()
        )
    ...
    def _agregar_otra_actividad(
        self, df_legajos: pd.DataFrame, df_otra_actividad: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def _agregar_codigos_obra_social(
        ...
        return df_legajos.merge(df_obra_social, on="nro_legaj", how="left").fillna(
            {"codigo_os": "000000"}
        )
    ...
    def _aplicar_topes(self, df_legajos: pd.DataFrame) -> pd.DataFrame:
        ...
        df_legajos.loc[mask_tope_sac, "DiferenciaSACImponibleConTope"] = (
            df_legajos.loc[mask_tope_sac, "ImporteSAC"]
            - self.config.tope_sac_jubilatorio_patr
        )
        df_legajos.loc[mask_tope_sac, "ImporteImponiblePatronal"] -= df_legajos.loc[
            mask_tope_sac, "DiferenciaSACImponibleConTope"
        ]
        df_legajos.loc[mask_tope_sac, "ImporteSACPatronal"] = (
            self.config.tope_sac_jubilatorio_patr
        )
    ...
    def _validar_legajos(self, df_legajos: pd.DataFrame) -> pd.DataFrame:
        ...
        mask_licencias = self.config.check_lic & (df_legajos["licencia"] == 1)
