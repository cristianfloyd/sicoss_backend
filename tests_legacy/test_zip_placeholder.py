...
def test_zip_placeholder():
    ...
    try:
        ruta_zip_creado = compressor.crear_zip_sicoss(
            archivos_txt=archivos_txt_simulados, periodo=periodo_test
        )
    ...
    archivos_eliminados = compressor.limpiar_archivos_temporales(
        "storage/comunicacion/sicoss"
    )
...
def test_integracion_processor():
    ...
    try:
        ...
        resultado = processor.procesar_datos_extraidos(
            datos=datos_simulados, crear_zip=True, nombre_archivo="test_integracion"
        )
