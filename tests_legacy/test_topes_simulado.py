...
def test_topes_simulados():
    ...
    df_test = pd.DataFrame(
        {
            ...
            "ImporteSAC": [
                ...
                1_000_000.0,
                500_000.0,
            ],  # [Excede, Normal, Normal]
            "ImporteAdicionales": [
                500_000.0,
                5_000_000.0,
                800_000.0,
            ],  # [Normal, Excede, Normal]
    ...
    df_test["IMPORTE_BRUTO"] = (
        df_test["ImporteImponiblePatronal"] + df_test["ImporteNoRemun"]
    )
    ...
    for _, row in df_test.iterrows():
        ...
        print(
            f"   ImporteImponiblePatronal: ${row['ImporteImponiblePatronal']:>12,.2f}"
        )
    ...
    for i, (_, row_antes) in enumerate(df_test.iterrows()):
        ...
        # Comparar importes
        print(
            f"   ImporteSACPatronal:       ${row_despues['ImporteSACPatronal']:>12,.2f}"
        )
        print(
            f"   ImporteImponiblePatronal: ${row_despues['ImporteImponiblePatronal']:>12,.2f}"
        )
        print(
            f"   ImporteImponibleSinSAC:   ${row_despues['ImporteImponibleSinSAC']:>12,.2f}"
        )
        ...
        # Cambios en importes
        cambio_patronal = (
            row_despues["ImporteImponiblePatronal"]
            - row_antes["ImporteImponiblePatronal"]
        )
    ...
    imponible_excede = (
        df_test.iloc[1]["ImporteImponibleSinSAC"] > config.tope_jubilatorio_patronal
    )
    tope_imponible_aplicado = caso2["DiferenciaImponibleConTope"] > 0

    print(
        f"✅ Caso 2 (Imponible excede): {imponible_excede} → Tope aplicado: {tope_imponible_aplicado}"
    )

    # Caso 3: Sin topes
    caso3 = df_con_topes.iloc[2]
    sin_topes = (
        caso3["DiferenciaSACImponibleConTope"] == 0
        and caso3["DiferenciaImponibleConTope"] == 0
    )
    ...
    if tope_sac_aplicado and tope_imponible_aplicado and sin_topes:
        print(
            "🎉 ¡TODAS LAS VALIDACIONES PASARON! TopesProcessor funciona correctamente"
        )
    else:
