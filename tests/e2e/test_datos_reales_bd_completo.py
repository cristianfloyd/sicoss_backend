...
class GeneradorDatosReales:
    ...
    def generar_dataset_realista(self, num_legajos: int) -> Dict:
        ...
        probabilidades = [
            self.escalafones_reales[esc]["porcentaje"] for esc in escalafones
        ]

        escalafones_asignados = np.random.choice(
            escalafones, size=num_legajos, p=probabilidades
        )
        ...
        print(
            f"   - Distribución escalafones: {df_legajos['escalafon'].value_counts().to_dict()}"
        )
    ...
    def _generar_conceptos_realistas(self, df_legajos: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, legajo in df_legajos.iterrows():
            ...
            sueldo_base = self._calcular_sueldo_base_realista(
                escalafon, categoria, dedicacion
            )
            ...
            for codn_conce, config in conceptos_config.items():
                if np.random.random() < config["prob"]:
                    ...
                    factor_variacion = np.random.uniform(
                        1 - variabilidad * 0.1, 1 + variabilidad * 0.1
                    )
    ...
    def _calcular_sueldo_base_realista(
        self, escalafon: str, categoria: int, dedicacion: str
    ) -> float:
    ...
    def _generar_otra_actividad_realista(
        ...
        for _, legajo in legajos_con_otra_actividad.iterrows():
            otra_actividad = {
                ...
                "importe_bruto_otra_actividad": round(
                    np.random.uniform(15000, 50000), 2
                ),
                "importe_imponible_otra_actividad": round(
                    np.random.uniform(12000, 40000), 2
                ),
            }
...
def test_procesamiento_datos_reales():
    ...
    try:
        ...
        datos_reales = generador.generar_dataset_realista(
            100
        )  # 100 empleados realistas
        ...
        if (
            ...
            and "legajos_procesados" in resultado
            and not resultado["legajos_procesados"].empty
        ):
...
def test_insercion_bd_real(resultado_procesamiento: Dict):
    ...
    try:
        ...
        print(
            f"🔄 Insertando en BD PostgreSQL - Período: {periodo_fiscal.year}/{periodo_fiscal.month:02d}"
        )

        # Ejecutar inserción en BD
        start_time = time.time()

        resultado_bd = processor._guardar_en_bd_sicoss(
            resultado_procesamiento, periodo_fiscal
        )
        ...
        if isinstance(resultado_bd, dict) and resultado_bd.get("success", False):
            ...
            print(
                f"   - Registros insertados: {resultado_bd.get('registros_insertados', 0)}"
            )
            print(
                f"   - Tabla destino: {resultado_bd.get('tabla_destino', 'suc.afip_mapuche_sicoss')}"
            )

            # Validar inserción con consulta directa
            return validar_insercion_bd(
                periodo_fiscal, resultado_bd.get("registros_insertados", 0)
            )

        else:
            error_msg = (
                resultado_bd.get("error", "Error desconocido")
                if isinstance(resultado_bd, dict)
                else str(resultado_bd)
            )
...
def validar_insercion_bd(
    periodo_fiscal: PeriodoFiscal, registros_esperados: int
) -> bool:
    ...
    try:
        ...
        if not resultado_df.empty:
            ...
            if total_registros == registros_esperados:
                ...
                print(
                    f"⚠️ Discrepancia: {total_registros} insertados vs {registros_esperados} esperados"
                )
                db_conn.close()
                return False
        else:
...
def main():
    ...
    print(
        f"   Paso 1 - Procesamiento Datos Reales: {'✅ EXITOSO' if exito_procesamiento else '❌ FALLÓ'}"
    )
