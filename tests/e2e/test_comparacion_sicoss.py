...
class ComparadorSicoss:
    ...
    def _definir_campos_sicoss(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "cod_mod_cont": {
                "nombre": "Código Modalidad Contractual",
                "tipo": "configuracion",
            },
            ...
            "rem_total": {
                "nombre": "Remuneración Total (Campo 7)",
                "tipo": "remuneracion",
                "campo_sicoss": 7,
            },
            "rem_impo1": {
                "nombre": "Remuneración Imponible 1 (Campo 14)",
                "tipo": "remuneracion",
                "campo_sicoss": 14,
            },
            "rem_impo2": {
                "nombre": "Remuneración Imponible 2 (Campo 21)",
                "tipo": "remuneracion",
                "campo_sicoss": 21,
            },
            "rem_impo3": {
                "nombre": "Remuneración Imponible 3 (Campo 22)",
                "tipo": "remuneracion",
                "campo_sicoss": 22,
            },
            "rem_impo4": {
                "nombre": "Remuneración Imponible 4 (Campo 23)",
                "tipo": "remuneracion",
                "campo_sicoss": 23,
            },
            "rem_impo5": {
                "nombre": "Remuneración Imponible 5 (Campo 42)",
                "tipo": "remuneracion",
                "campo_sicoss": 42,
            },
            "rem_impo6": {
                "nombre": "Remuneración Imponible 6 (Campo 44)",
                "tipo": "remuneracion",
                "campo_sicoss": 44,
            },
            "rem_imp7": {
                "nombre": "Remuneración 7 (Campo 49)",
                "tipo": "remuneracion",
                "campo_sicoss": 49,
            },
            "rem_dec_788": {
                "nombre": "Remuneración 788/05 - Imponible 8 (Campo 48)",
                "tipo": "remuneracion",
                "campo_sicoss": 48,
            },
            "rem_imp9": {
                "nombre": "Remuneración Imponible 9 (Campo 54)",
                "tipo": "remuneracion",
                "campo_sicoss": 54,
            },
            ...
            "horas_extras": {
                "nombre": "Horas Extras (Campo 9)",
                "tipo": "concepto",
                "campo_sicoss": 9,
            },
            "zona_desfav": {
                "nombre": "Zona Desfavorable (Campo 10)",
                "tipo": "concepto",
                "campo_sicoss": 10,
            },
            "vacaciones": {
                "nombre": "Vacaciones (Campo 40)",
                "tipo": "concepto",
                "campo_sicoss": 40,
            },
            "adicionales": {
                "nombre": "Importe Adicionales (Campo 46)",
                "tipo": "concepto",
                "campo_sicoss": 46,
            },
            "premios": {
                "nombre": "Importe Premios (Campo 47)",
                "tipo": "concepto",
                "campo_sicoss": 47,
            },
            "cpto_no_remun": {
                "nombre": "Conceptos No Remunerativos (Campo 51)",
                "tipo": "concepto",
                "campo_sicoss": 51,
            },
            "maternidad": {
                "nombre": "Maternidad (Campo 52)",
                "tipo": "concepto",
                "campo_sicoss": 52,
            },
            "rectificacion_remun": {
                "nombre": "Rectificación de Remuneración (Campo 53)",
                "tipo": "concepto",
                "campo_sicoss": 53,
            },
            # Otros campos importantes
            "asig_fam_pag": {
                "nombre": "Asignaciones Familiares Pagadas",
                "tipo": "otros",
            },
            "aporte_vol": {"nombre": "Aportes Voluntarios", "tipo": "otros"},
            "regimen": {
                "nombre": "Régimen (Campo 29)",
                "tipo": "otros",
                "campo_sicoss": 29,
            },
            "convencionado": {
                "nombre": "Trabajador Convencionado (Campo 43)",
                "tipo": "otros",
                "campo_sicoss": 43,
            },
            "tipo_empresa": {
                "nombre": "Tipo de Empresa (Campo 27)",
                "tipo": "otros",
                "campo_sicoss": 27,
            },
            "seguro": {
                "nombre": "Seguro de Vida Obligatorio (Campo 57)",
                "tipo": "otros",
                "campo_sicoss": 57,
            },
        }
    ...
    def ejecutar_comparacion_completa(
        self, per_anoct: int = 2025, per_mesct: int = 5
    ) -> Dict:
        ...
        try:
            ...
            if not legajo_comun:
                raise Exception(
                    "No se encontró ningún legajo común entre las tablas Python y PHP Legacy"
                )
            ...
            resultado_python = self._ejecutar_pipeline_python(
                per_anoct, per_mesct, legajo_comun
            )

            if not resultado_python["success"]:
                raise Exception(
                    f"Error en pipeline Python: {resultado_python.get('error')}"
                )

            # PASO 3: Obtener datos del sistema legacy
            print(f"\nPASO 3: Obteniendo datos legacy para legajo {legajo_comun}...")
            datos_legacy = self._obtener_datos_legacy(
                legajo_comun, per_anoct, per_mesct
            )

            if datos_legacy.empty:
                raise Exception(
                    f"No se encontraron datos legacy para legajo {legajo_comun}"
                )

            # PASO 4: Obtener datos del sistema Python
            print(f"\nPASO 4: Obteniendo datos Python para legajo {legajo_comun}...")
            datos_python = self._obtener_datos_python(
                legajo_comun, per_anoct, per_mesct
            )

            if datos_python.empty:
                raise Exception(
                    f"No se encontraron datos Python para legajo {legajo_comun}"
                )

            # PASO 5: Comparar campo por campo
            print("\n PASO 5: Comparando campos SICOSS...")
            comparacion = self._comparar_datos(
                datos_python.iloc[0], datos_legacy.iloc[0]
            )
    ...
    def _ejecutar_pipeline_python(
        self, per_anoct: int, per_mesct: int, cuil_target: str
    ) -> Dict:
        ...
        try:
            ...
            if df_legajo.empty:
                return {
                    "success": False,
                    "error": f"No se encontró legajo para CUIL {cuil_target}",
                }
            ...
            if (
                "legajos_procesados" not in proc_resultado
                or proc_resultado["legajos_procesados"].empty
            ):
            ...
            return {
                ...
                "legajos_guardados": guardado.get("legajos_guardados", 0),
            }

        except Exception as e:
            return {"success": False, "error": str(e)}

    def _obtener_datos_legacy(
        self, cuil: str, per_anoct: int, per_mesct: int
    ) -> pd.DataFrame:
    ...
    def _comparar_datos(self, datos_python: pd.Series, datos_legacy: pd.Series) -> Dict:
        ...
        for campo, config in self.campos_sicoss.items():
            if campo in datos_python.index and campo in datos_legacy.index:
                ...
                if isinstance(valor_python, (int, float, Decimal)) and isinstance(
                    valor_legacy, (int, float, Decimal)
                ):
        ...
        resumen = {
            ...
            "porcentaje_coincidencia": (len(coincidencias) / len(self.campos_sicoss))
            * 100
            if self.campos_sicoss
            else 0,
        }
    ...
    def _generar_reporte_comparacion(self, resultado: Dict):
        ...
        if resultado["success"]:
            ...
            print(
                f"   - Porcentaje de coincidencia: {resumen['porcentaje_coincidencia']:.1f}%"
            )
            ...
            if resultado["diferencias"]:
                ...
                for diff in resultado["diferencias"]:
                    ...
                    print(
                        f"  Diferencia: {self._calcular_diferencia(diff['valor_python'], diff['valor_legacy'])}"
                    )
                    print()

            # Mostrar algunas coincidencias importantes
            coincidencias_importantes = [
                c
            ...
            if coincidencias_importantes:
                ...
                for coin in coincidencias_importantes[
                    :10
