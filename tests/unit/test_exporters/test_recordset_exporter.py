...
def crear_datos_prueba():
    ...
    resultado_sicoss = {
        ...
        "metricas": {"tiempo_total_segundos": 1.234},
    }
...
def test_inicializacion():
    ...
    try:
        ...
        assert exporter_basic is not None, "Exporter básico debe ser inicializado"
        print(" Exporter básico inicializado correctamente")

        # Con debug info
        exporter_debug = SicossRecordsetExporter(include_debug_info=True)
        assert exporter_debug is not None, "Exporter con debug debe ser inicializado"
...
def test_transformacion_completa():
    ...
    try:
        ...
        api_response = exporter.transformar_resultado_completo(
            resultado_sicoss, include_details=True
        )

        # Verificaciones básicas
        assert api_response.success, "Response debe ser exitosa"
        ...
        print(
            f"   - Primer legajo: {primer_legajo['nro_legaj']} - {primer_legajo['apnom']}"
        )
        ...
        assert estadisticas["tiempo_procesamiento_ms"] > 0, (
            "Debe tener tiempo de procesamiento"
        )

        print(
            f"   - Tiempo procesamiento: {estadisticas['tiempo_procesamiento_ms']:.0f}ms"
        )
...
def test_formatos_respuesta():
    ...
    try:
        ...
        for formato in formatos:
            ...
            # Verificaciones básicas
            assert resultado_laravel["success"], f"Formato {formato} debe ser exitoso"
            assert "data" in resultado_laravel, f"Formato {formato} debe tener data"
            assert "metadata" in resultado_laravel, (
                f"Formato {formato} debe tener metadata"
            )
            ...
            if formato == "solo_totales":
                assert "totales" in resultado_laravel["data"], (
                    "Solo totales debe tener totales"
                )
                assert "resumen" in resultado_laravel["data"], (
                    "Solo totales debe tener resumen"
                )
                legajos_count = len(resultado_laravel["data"].get("legajos", []))
                print(
                    f"         - Solo datos esenciales: {len(resultado_laravel['data'])} secciones"
                )
...
def test_respuesta_fastapi():
    ...
    try:
        ...
        assert resultado_fastapi["success"], "FastAPI response debe ser exitosa"
        ...
        assert resultado_fastapi["content_type"] == "application/json", (
            "Content type debe ser JSON"
        )

        print("Respuesta FastAPI generada correctamente")
        ...
        print(
            f"   - Processing time: {resultado_fastapi['metadata']['processing_time_ms']:.0f}ms"
        )
...
def test_manejo_errores():
    ...
    try:
        ...
        datos_invalidos = {"datos_corruptos": "test", "sin_legajos": True}

        respuesta_error = exporter.transformar_resultado_completo(datos_invalidos)

        # Verificar que maneja el error correctamente
        assert not respuesta_error.success, "Debe fallar con datos inválidos"
        assert "Error en procesamiento SICOSS" in respuesta_error.message, (
            "Debe tener mensaje de error"
        )
        assert respuesta_error.data.get("error_details"), (
            "Debe tener detalles del error"
        )
...
def test_integracion_processor():
    ...
    try:
        ...
        assert hasattr(processor, "recordset_exporter"), (
            "Processor debe tener recordset_exporter"
        )
        assert processor.recordset_exporter is not None, (
            "recordset_exporter no debe ser None"
        )
        ...
        assert api_response["success"], "API response debe ser exitosa"
        assert "legajos" in api_response["data"], "API response debe tener legajos"
        assert api_response["metadata"]["backend"] == "sicoss_python", (
            "Debe tener metadata correcto"
        )

        print("API response generada en pipeline:")
        print(f"   - Success: {api_response['success']}")
        print(f"   - Legajos: {len(api_response['data']['legajos'])}")
        ...
        assert respuesta_directa["content_type"] == "application/json", (
            "Debe tener content type"
        )

        print("Método directo funcional:")
...
def test_performance_basico():
    ...
    try:
        ...
        resultado_grande = {
            ...
            "metricas": {"tiempo_total_segundos": 2.5},
        }
        ...
        assert resultado_api.success, "Transformación debe ser exitosa"
        assert len(resultado_api.data["legajos"]) == 100, "Debe procesar 100 legajos"

        print("Performance test completado:")
