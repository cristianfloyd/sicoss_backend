...
class ConceptosProcessorTester:
    ...
    def test_legajo(
        ...
        logger.info(
            f"🧪 Probando legajo {nro_legaj} para período {per_anoct}/{per_mesct}"
        )

        try:
            # Verificar que los componentes estén inicializados
            if (
                not self.legajos_extractor
                or not self.conceptos_extractor
                or not self.conceptos_processor
            ):
                return {
            ...
            return self._analizar_resultados(
                nro_legaj, df_legajos, df_conceptos, df_resultado
            )
        ...
        for campo in campos_importantes:
            if campo in legajo_data:
                ...
                if pd.notna(valor) and valor != 0:
                    campos_sicoss[campo] = (
                        float(valor) if isinstance(valor, (int, float)) else valor
                    )

        # Estadísticas
        estadisticas = {
            "total_conceptos": len(df_conceptos),
            "total_importe": float(df_conceptos["impp_conce"].sum())
            if not df_conceptos.empty
            else 0,
            "campos_con_valor": len(campos_sicoss),
            "tipos_grupos_únicos": list(
                set(
                    [
                        tipo
                        for tipos in df_conceptos["tipos_grupos"].tolist()
    ...
    def mostrar_resultados(self, resultados: Dict[str, Any]):
        ...
        if resultados["conceptos_entrada"]:
            ...
            for concepto in resultados["conceptos_entrada"]:
                print(
                    f"   {concepto['codn_conce']:>6} | {concepto['tipo_conce']} | "
...
def parse_arguments():
    ...
    parser.add_argument(
        "--legajo", "-l", type=int, required=True, help="Número de legajo a probar"
    )
