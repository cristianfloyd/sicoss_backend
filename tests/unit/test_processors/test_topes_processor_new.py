...
class TopesProcessorTester:
    ...
    def setup(self):
        ...
        try:
            ...
            logger.info(
                f"   💰 Tope SAC patronal: ${tope_jubilatorio_patronal / 2:,.2f}"
            )
            logger.info(
        ...
        try:
            ...
            return {
                ...
                "topes_aplicados": self._analizar_topes_aplicados(
                    datos_antes.iloc[0], df_con_topes.iloc[0]
                ),
            }
    ...
    def _preparar_datos_para_topes(self, df: pd.DataFrame):
        ...
        logger.info(
            f"📊 Datos preparados - SAC: ${df.iloc[0]['ImporteSAC']:.2f}, Imponible: ${df.iloc[0]['ImporteImponiblePatronal']:.2f}"
        )
...
def mostrar_resultados(resultados: Dict[str, Any]):
    ...
    print("-" * 60)
    print(
        f"   Tope Jubilatorio Patronal  | ${topes_config['tope_jubilatorio_patronal']:>15,.2f}"
    )
    print(
        f"   Tope SAC Patronal          | ${topes_config['tope_sac_patronal']:>15,.2f}"
    )
    print(
        f"   Tope Jubilatorio Personal  | ${topes_config['tope_jubilatorio_personal']:>15,.2f}"
    )
    ...
    if topes["importes_cambiados"]:
        print(
            f"   🔄 Campos modificados      | {', '.join(topes['importes_cambiados'])}"
        )
