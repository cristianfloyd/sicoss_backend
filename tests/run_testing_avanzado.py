...
def ejecutar_test_individual(nombre_test: str, archivo_test: str) -> bool:
    ...
    try:
        ...
        if archivo_test == "test_sicoss_verifier_avanzado.py":
            from tests.unit.test_validators.test_sicoss_verifier_avanzado import (
                main as test_main,
            )
        elif archivo_test == "test_performance_masivo.py":
            from tests.performance.test_performance_masivo import main as test_main
        elif archivo_test == "test_datos_reales_produccion.py":
            from tests.e2e.test_datos_reales_produccion import main as test_main
        else:
    ...
    try:
        ...
        if (
            ...
            campos_presentes = sum(
                1 for campo in campos_criticos if campo in df_resultado.columns
            )
            ...
            if isinstance(resultado, dict):
                if "legajos_procesados" not in resultado:
                    print(
                        f"❌ Estructura incorrecta - Keys disponibles: {list(resultado.keys())}"
                    )
                elif resultado["legajos_procesados"].empty:
                    print("❌ Procesamiento sin resultados - DataFrame vacío")
                else:
