...
class PerformanceProfiler:
    ...
    def finalizar_medicion(self, nombre: str, datos_procesados: int) -> Dict:
        ...
        if self.start_time is None:
            raise ValueError(
                "start_time no ha sido inicializado. Llama a iniciar_medicion() primero."
            )
        ...
        metricas = {
            ...
            "tiempo_por_registro": elapsed_time / datos_procesados
...
def generar_datos_masivos(num_legajos: int, conceptos_por_legajo: int = 5) -> Dict:
    ...
    print(
        f"Generando {num_legajos:,} legajos con ~{conceptos_por_legajo} conceptos cada uno..."
    )
    ...
    for legajo_id in df_legajos["nro_legaj"]:
        ...
        conceptos_legajo = np.random.choice(
            tipos_conceptos,
            size=min(num_conceptos, len(tipos_conceptos)),
            replace=False,
        )
...
def test_performance_escalabilidad():
        ...
        try:
            ...
            if resultado["success"]:
                metricas = profiler.finalizar_medicion(
                    f"Procesamiento_{tamaño}_legajos", tamaño
                )
            ...
            resultados_escalabilidad.append(
                {"tamaño_dataset": tamaño, "success": False, "error": str(e)}
            )
    ...
    resultados_exitosos = [
        r for r in resultados_escalabilidad if r.get("success", False)
    ]
    ...
    try:
        ...
        for nombre, metricas in resultados_componentes.items():
            if "error" not in metricas:
                print(f"{nombre}:")
                print(f"    Tiempo: {metricas['tiempo_total']:.3f}s")
                print(
                    f"    Throughput: {metricas['throughput_por_segundo']:.1f} legajos/s"
                )
...
def main():
    ...
    print(f"Tiempo total tests: {elapsed_time:.2f}s")
    ...
    if tests_exitosos == len(tests):
        ...
        print(f"\n{len(tests) - tests_exitosos} tests fallaron")
