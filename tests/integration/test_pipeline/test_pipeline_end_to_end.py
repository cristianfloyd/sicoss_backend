...
class TestPipelineCompleto:
    ...
    def _cargar_configuracion_real(self) -> SicossConfig:
        ...
        config = SicossConfig(
            ...
            tope_otros_aportes_personales=topes.get(
                "TopeOtrosAportesPersonales", 3245240.49
            ),
            ...
            asignacion_familiar=otros_params.get(
                "AcumularAsigFamiliar", "false"
            ).lower()
            == "true",
        )
        ...
        print(
            f"   - Tope Jubilatorio Personal: ${config.tope_jubilatorio_personal:,.2f}"
        )
        print(
            f"   - Tope Jubilatorio Patronal: ${config.tope_jubilatorio_patronal:,.2f}"
        )
    ...
    def ejecutar_test_completo(
        self, per_anoct: int = 2025, per_mesct: int = 5, limite: int = 100
    ) -> Dict:
        ...
        print(
            f"🎯 Estrategia: Procesamiento masivo de {limite} legajos con datos reales"
        )
        print("=" * 60)
        ...
        try:
            ...
            if not datos["legajos"].empty and not datos["conceptos"].empty:
                ...
                stats_conceptos = (
                    datos["conceptos"]
                    .groupby("codn_conce")
                    .agg({"impp_conce": ["count", "sum"]})
                    .head(5)
                )
                print(f"📈 Top 5 conceptos más frecuentes:")
                for idx, row in stats_conceptos.iterrows():
                    print(
                        f"   Concepto {idx}: {row[('impp_conce', 'count')]} veces, ${row[('impp_conce', 'sum')]:,.2f}"
                    )
            ...
            if "legajos_procesados" not in proc_resultado:
                raise Exception(
                    f"Error procesamiento: formato inesperado - {list(proc_resultado.keys())}"
                )
            ...
            if (
                "api_response" in proc_resultado
                and "success" in proc_resultado["api_response"]
            ):
            ...
            verificacion = self._verificar_guardado(
                periodo, guardado["legajos_guardados"]
            )
            t4 = time.time() - t4
    ...
    def _verificar_guardado(
        self, periodo: PeriodoFiscal, esperados: int
    ) -> Dict[str, bool]:
...
def main():
    ...
    try:
        ...
        if resultado["success"]:
            ...
            print(
                f"🎯 Legajo procesado exitosamente: {resultado.get('legajo_procesado', 'N/A')}"
            )
