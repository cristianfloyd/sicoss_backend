...
class LicenciasExtractor(BaseExtractor):
    ...
    def extract(self, **kwargs: Any) -> pd.DataFrame:
        ...
        if not isinstance(periodo, PeriodoFiscal):
            ...
            if anio and mes:
                ...
                raise ValueError("Se requiere un objeto PeriodoFiscal o per_anoct/per_mesct")
    ...
    def extract_for_legajos(
        ...
        if variantes_vacaciones:
            where_vacaciones = f" WHEN dh05.nrovarlicencia IN ({variantes_vacaciones}) THEN 12 "
            filtro_variantes.append(variantes_vacaciones)
        
        if variantes_protecintegral:
            where_protecintegral = f" WHEN dh05.nrovarlicencia IN ({variantes_protecintegral}) THEN 51 "
        ...
        try:
            ...
            logger.info(f"✅ Se extrajeron {len(df)} licencias para {len(legajos_ids)} legajos")
            return df
