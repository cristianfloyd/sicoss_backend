...
class LegajosExtractor(BaseExtractor):
    ...
    def extract(
        self, per_anoct: int, per_mesct: int, where_legajo: str = "true", **kwargs
    ) -> pd.DataFrame:
        """Extrae datos básicos de legajos"""
        self._validate_params(
            ["per_anoct", "per_mesct"], per_anoct=per_anoct, per_mesct=per_mesct
        )

        logger.info(f"Extrayendo legajos para período {per_anoct}/{per_mesct}")

        query = self.queries.get_legajos_query(
            per_anoct, per_mesct, "'REPA'", where_legajo
        )
        return self.db.execute_query(query)

    def extract_for_legajo(
        self, per_anoct: int, per_mesct: int, nro_legajo: int
    ) -> pd.DataFrame:
