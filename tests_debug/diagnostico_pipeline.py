...
def diagnostico_completo():
    ...
    try:
        ...
        if df_conceptos.empty:
            ...
            if not df_otros.empty:
                ...
                for _, row in df_otros.iterrows():
                    print(
                        f"   - {int(row['per_anoct'])}/{int(row['per_mesct']):02d}: {int(row['num_conceptos'])} conceptos"
                    )
            ...
            for _, row in df_conceptos.head(10).iterrows():
                print(
                    f"   - Legajo {int(row['nro_legaj'])}: Concepto {int(row['codn_conce'])} = ${float(row['impp_conce']):,.2f}"
                )
        ...
        datos_extraidos = extractor.extraer_datos_completos(
            config=config, per_anoct=2025, per_mesct=5
        )
        ...
        if not datos_extraidos["conceptos"].empty:
            print(
                f"   Columnas en conceptos: {list(datos_extraidos['conceptos'].columns)}"
            )
        else:
        ...
        if not df_alternativo.empty:
            ...
            datos_test = extractor.extraer_datos_completos(
                config=config, per_anoct=mejor_ano, per_mesct=mejor_mes
            )
            ...
            if len(datos_test["conceptos"]) > 0:
                print(
                    f"   🎉 ¡PERÍODO FUNCIONAL ENCONTRADO! Usar {mejor_ano}/{mejor_mes:02d}"
                )
