...
class DiagnosticoCodAct:
    ...
    def ejecutar_diagnostico(
        self, per_anoct: int = 2025, per_mesct: int = 5, limite_legajos: int = 20
    ):
        ...
        legajos, conceptos = self._obtener_datos_prueba(
            per_anoct, per_mesct, limite_legajos
        )
    ...
    def _obtener_datos_prueba(self, per_anoct: int, per_mesct: int, limite: int):
        ...
        if df_legajos.empty:
            print(
                "⚠️ No se encontraron legajos con conceptos investigador, usando muestra general..."
            )
    ...
    def _analizar_diferencias(self, python_data: pd.DataFrame, php_data: pd.DataFrame):
        ...
        comparacion = python_data.merge(
            php_data[["nro_legaj", "TipoDeActividad_PHP"]], on="nro_legaj", how="inner"
        )
        ...
        print(
            f"   - Coincidencias cod_act: {coincidencias}/{total} ({porcentaje:.1f}%)"
        )
        ...
        if coincidencias < total:
            ...
            for _, row in diferencias.head(5).iterrows():
                print(
                    f"   Legajo {row['nro_legaj']}: Python={row['TipoDeActividad']}, PHP={row['TipoDeActividad_PHP']}"
                )
                print(
                    f"     codigoactividad={row['codigoactividad']}, prioridad={row['PrioridadTipoDeActividad']}"
                )
    ...
    def _diagnostico_detallado(self, resultado: pd.DataFrame, conceptos: pd.DataFrame):
        ...
        caso2 = resultado[
            (
                (resultado["PrioridadTipoDeActividad"] >= 34)
                & (resultado["PrioridadTipoDeActividad"] <= 37)
            )
        ...
        if len(caso2) > 0:
            verificacion2 = (
                caso2["TipoDeActividad"] == caso2["PrioridadTipoDeActividad"]
            ).all()
            print(f"     ✅ Lógica correcta: {verificacion2}")

        # Verificar conceptos investigador
        tipos_investigador = [11, 12, 13, 14, 15, 48, 49]
        conceptos_inv = conceptos[
            conceptos["tipos_grupos"].str.contains(
                "|".join(map(str, tipos_investigador)), na=False
            )
        ]
